...
class OPESM4 {
 public:
    OPESM4(const std::string &keyarg, size_t plainbits, size_t cipherbits)
    : key(keyarg), pbits(plainbits), cbits(cipherbits), block_key(seed(key)){}//哈希处理密钥，使得可以输入不同长度的密钥
...
class OPEWBSM4 {
 public:
    OPEWBSM4(const std::string &keyarg, size_t plainbits, size_t cipherbits)
    : key(keyarg), pbits(plainbits), cbits(cipherbits), block_key(seed(key)){}//哈希处理密钥，使得可以输入不同长度的密钥
